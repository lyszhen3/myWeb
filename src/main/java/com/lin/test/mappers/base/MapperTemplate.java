    ...
    public void setSqlSource(MappedStatement ms) throws Exception{
        if(mapperClass==getMapperClass(ms.getId())){
            ...
            if(this.mapperHelper.isSpring()){
                throw new RuntimeException("String 4.x.x 及以上版本支持泛型注入，您当前的Spring版本为"+mapperHelper.getSpringVersion()+",不能使用泛型注入，因此在配置MapperScannerConfigurer时,不要扫描通用Mapper接口类,也不要在您Mybatis的xml配置文件中的<mappers>中指定通用Mapper接口类");
            }
