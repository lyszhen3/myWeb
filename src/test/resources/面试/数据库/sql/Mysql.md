...
3. Isolation(隔离性)
    ...
    2. 读已提交
        1. 解决了`脏读`  
            读到的数据被回滚了
        2. 不可重读读,幻读
        3. 读事务允许读事务和写事务,未提交的写事务禁止读事务和写事务
           * mvcc的实现模式,读事务和写事务不会互相阻塞
           * 更新数据会有行锁
        4. mvcc
            * 每次select重新生成readView
    3. 可重复读
        1. 解决了`不可重复读`  
            第一次读到的数据和第二次(数据被更新)读到的数据不一致
        2. 幻读
        3. 读事务允许读事务禁止写事务, 未提交的写事务禁止读事务和写事务
          * mvcc的实现的实现模式, 读事务和写事务不会互相阻塞
          * 更新数据会有行锁
        1. mvcc
           * 第一次select生成readView
    4. 序列化
